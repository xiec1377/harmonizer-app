...
export function UploadStage() {
  ...
  const { setStage, setAudioAnalysis, setAudioFile, setHarmonyAnalysis, setError, error } = useStore()
    ...
    <div className="flex flex-col items-center justify-center min-h-[60vh] gap-8 animate-fade-up">
      ...
      <div className="text-center space-y-3">
        ...
        <p className="font-body text-silver text-lg max-w-md mx-auto">
          Upload any song. Our AI extracts the melody and builds harmony voices tailored to the piece — then listens as you sing along.
        </p>
      </div>
      ...
      >
        <input ref={inputRef} type="file" className="hidden" accept=".mp3,.wav,.ogg,.flac,.m4a,.aac" onChange={onFileChange} />
        <div className="flex flex-col items-center gap-4 py-14 px-8">
          <div className={`p-4 rounded-full transition-colors ${dragging ? 'bg-resonance/20' : 'bg-mist'}`}>
            <Upload className={`w-8 h-8 ${dragging ? 'text-resonance' : 'text-silver'}`} />
          </div>
          <div className="text-center">
            <p className="text-pearl font-medium">Drop an audio file here</p>
            <p className="text-silver text-sm mt-1">MP3, WAV, FLAC, OGG, M4A — up to 50 MB</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-3 flex-wrap justify-center">
        {['MP3', 'WAV', 'FLAC', 'OGG', 'M4A'].map((fmt) => (
          <span key={fmt} className="px-3 py-1 rounded-full bg-mist text-silver text-xs font-mono border border-slate/50">
            {fmt}
          </span>
        ))}
      </div>
      ...
      <div className="flex gap-4 flex-wrap justify-center mt-2">
        {[
          { icon: '♩', label: 'Note extraction' },
          { icon: '🤖', label: 'AI harmony voices' },
          { icon: '🎤', label: 'Real-time pitch match' },
        ].map(({ icon, label }) => (
          <div key={label} className="flex items-center gap-2 text-silver text-sm">
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] gap-6 animate-fade-up">
      ...
      <div className="text-center space-y-2">
        <p className="text-pearl font-display text-2xl">Listening to your music...</p>
        <p className="text-silver text-sm">Extracting melody, chords, key — then asking the AI to compose harmony voices</p>
      </div>
      {/* Fake progress bar */}
